...
export function filterProjects (rows, options = {}) {
  ...
  rows = _.filter(rows, row => {
    ...
    // Exclude blacklisted projects if requested
    if (atom.config.get('project-plus.blacklist')) {
      const pattern = new RegExp(atom.config.get('project-plus.blacklist'))
      if (pattern.test(row.paths)) return false
    }
